...
def set_geme_location(location: typing.Optional[PathLike]):
    """Sets game dir"""
    ...
    if log.is_file():
        GAME_LOCATION.write_text(location)
    else:
        raise FileNotFoundError("Incorrect game location, remember to set it before \"Genshin Impact game\" folder!")
...
def _read_logfile(logfile: typing.Optional[PathLike] = None) -> str:
    ...
    try:
        return logfile.read_text(errors='replace') # won't work if genshin is running or script using this function is run as administrator
    except PermissionError:
        print("Pleas turn off genshin impact or try running script as administrator!")
        return "no authkey"

def extract_authkey(string: str) -> typing.Optional[str]:
    """Extract an authkey from the provided string."""
    match = re.search(r"https://webstatic-sea.hoyoverse.com/genshin/event/e20190909gacha-v2.+?authkey=([^&#]+)", string, re.MULTILINE)
